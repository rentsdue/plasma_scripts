...
if __name__ == "__main__":
    ...
    axs[0].set_title(r"Fig 7a: True Folded Log $E_{\text{zonal}}(|k_x|)$", fontsize=11)
    axs[0].set_xlabel("Wavenumber Magnitude $|m|$ (1 to 64)")
    axs[0].set_ylabel(r"$\log_{10} C$")
    fig.colorbar(im0, ax=axs[0], label="Log Power Intensity")
    ...
    axs[1].set_title("Fig 7b: LOOCV Mode Prediction (Folded)", fontsize=11)
    axs[1].set_xlabel("Wavenumber Magnitude $|m|$ (1 to 64)")
    axs[1].set_ylabel(r"$\log_{10} C$")
    fig.colorbar(im1, ax=axs[1], label="Log Power Intensity")
    
    # Panel 7c: Comparative Multi-Family Error Panel with Baseline Overlay
    axs[2].plot(log10_c_axis, df['NN_Global'].values, color='black', marker='o', lw=2, label='Global NN Error')
    axs[2].plot(log10_c_axis, df['Base_Global'].values, color='black', linestyle='--', alpha=0.6, label='Global Baseline')
    
    axs[2].plot(log10_c_axis, df['NN_Ex'].values, color='#1f77b4', marker='s', alpha=0.8, label='E(kx) NN')
    axs[2].plot(log10_c_axis, df['Base_Ex'].values, color='#1f77b4', linestyle=':', alpha=0.5)
    
    axs[2].plot(log10_c_axis, df['NN_Zf'].values, color='#2ca02c', marker='^', alpha=0.8, label='E_Zonal NN')
    axs[2].plot(log10_c_axis, df['Base_Zf'].values, color='#2ca02c', linestyle=':', alpha=0.5)
    
    axs[2].set_title("Fig 7c: Sub-Family Error vs Baseline Comparison", fontsize=11)
    axs[2].set_xlabel(r"$\log_{10} C$")
    axs[2].set_ylabel("Median Absolute Deviation [Decades]")
