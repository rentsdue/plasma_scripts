...
if __name__ == "__main__":
    ...
    plt.errorbar(df['C_Value'], df['True_RMS'], yerr=df['RMS_Std'], fmt='o', color='black', label='True Mean $\pm \sigma_t$', capsize=4)
    plt.plot(df['C_Value'], df['NN_Pred_RMS'], 'r-x', label='NN Forecast')
    plt.plot(df['C_Value'], df['Base_Pred_RMS'], 'b--.', label='Linear Spline Baseline')
    plt.xscale('log')
    plt.yscale('log')
    plt.title("Zonal Flow Amplitude RMS(C) Profile with Turbulent Fluctuations Floor")
    plt.xlabel("Adiabatic parameter $C$")
    plt.ylabel("Zonal Flow RMS Magnitude")
    plt.legend()
